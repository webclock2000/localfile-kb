"""系统状态页面 — 健康检查、失败队列、运行记录、实时日志。

参考设计: RAGFlow Admin UI 服务状态面板。
"""

import requests
import streamlit as st

API_BASE = "http://localhost:9494"

st.set_page_config(page_title="FileKB — 系统状态", page_icon="⚙️", layout="wide")

st.title("⚙️ 系统状态")

try:
    resp = requests.get(f"{API_BASE}/status", timeout=10)
    if resp.status_code == 200:
        data = resp.json()
        health = data.get("health", {})

        # 健康检查卡片
        st.subheader("服务健康")

        hc1, hc2, hc3, hc4 = st.columns(4)
        with hc1:
            llm_status = health.get("llm_server", "unknown")
            st.metric("LLM 服务", "✅ 在线" if llm_status == "ok" else "❌ 离线")
        with hc2:
            sqlite = health.get("sqlite", {})
            st.metric("数据库", f"{sqlite.get('size_mb', 0):.1f} MB")
        with hc3:
            faiss = health.get("faiss", {})
            st.metric("FAISS 索引", f"{faiss.get('vectors', 0)} 向量")
        with hc4:
            graph_h = health.get("graph", {})
            st.metric("知识图谱", f"{graph_h.get('nodes', 0)} 节点 / {graph_h.get('edges', 0)} 边")

        # 最近运行
        last = data.get("last_run")
        if last:
            st.subheader("最近索引运行")
            st.json(last)

        # 操作
        st.subheader("操作")
        c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("🔄 立即索引"):
                requests.post(f"{API_BASE}/index", json={}, timeout=5)
                st.success("索引已启动")
        with c2:
            if st.button("📋 重试失败队列"):
                requests.post(f"{API_BASE}/index", json={"dlq": True}, timeout=5)
                st.success("DLQ 重试已启动")
        with c3:
            if st.button("🔄 刷新状态"):
                st.rerun()

        # 实时日志
        st.subheader("📋 运行日志")
        c4, c5 = st.columns([1, 3])
        with c4:
            log_level = st.selectbox("日志级别", ["DEBUG", "INFO", "WARNING", "ERROR"], index=1)
        with c5:
            log_lines = st.slider("行数", 10, 200, 50)

        try:
            log_resp = requests.get(
                f"{API_BASE}/logs",
                params={"lines": log_lines, "level": log_level},
                timeout=5,
            )
            if log_resp.status_code == 200:
                entries = log_resp.json().get("lines", [])
                if entries:
                    log_text = "\n".join(e["text"] for e in entries)
                    st.code(log_text, language=None)
                else:
                    st.caption("没有匹配的日志条目。")
        except Exception:
            st.caption("无法加载日志。")

    else:
        st.error(f"API 错误: {resp.status_code}")

except requests.exceptions.ConnectionError:
    st.error(
        "无法连接到 FileKB 后端服务。请先启动服务：\n\n"
        "```bash\n"
        "filekb ui\n"
        "```"
    )
except Exception as e:
    st.error(f"出错了: {e}")
